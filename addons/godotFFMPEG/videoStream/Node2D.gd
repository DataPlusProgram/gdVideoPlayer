...
export var color2 : Color = Color.dodgerblue
...
func _ready():
	...
	AudioServer.add_bus_effect(0,e)
...
func _draw():
	...
	#draw_line(Vector2(0, -total_h), Vector2(total_w, -total_h), color1, 2.0, true)
	
	for i in range(definition):
		draw_line(draw_pos, draw_pos + Vector2(0, -histogram[i] * total_h), color1, 4.0, true)
